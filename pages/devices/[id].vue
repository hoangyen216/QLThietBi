<template>
  <h2>Edit Device</h2>
  <el-form ref="ruleFormRef" :model="ruleForm" label-width="150px" style="width: 50% " class="demo-ruleForm"
...
const updateForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      console.log('submit!')
      const { data, error, status } = await useFetch(`https://65183f73582f58d62d358659.mockapi.io/devices/${route.params.id}`, {
        method: 'PUT',
        body: ruleForm
      })
