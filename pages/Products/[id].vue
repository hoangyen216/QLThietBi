    ...
    <div>
      <div class="row mb-5">
        <div class="col6 col-xl-6 col-lg-6 col-md-12 col-sm-12">
          <img class="img-fluid" :src="dataDetailDevice.image">
        </div>
        ...
        <div class="col6 col-xl-6 col-lg-6 col-md-12 col-sm-12 d-flex align-items-center justify-content-start">
          <div class="info pt-xl-0 pt-lg-0 pt-5">
            ...
            <h1 class="font-weight-bold text-uppercase pt-3">{{ dataDetailDevice.deviceDescr }} </h1>
            <h4>Số lượnng:{{ dataDetailDevice.currentAmount }}</h4>
            <br><br><br>
            ...
            <div class="card-actions" style="margin-bottom: 20px; width: 156px;">
              <div class="btn-group" style="border: 1px solid #0d6efd;">
                <button class="btn btn-primary" @click="decrememnt">
                  -
                </button>
                <button class="btn btn-ghost no-animation">
                  {{ quantity }}
                </button>
                <button class="btn btn-primary"  @click="incrememnt">
                  +
                </button>
              </div>
            </div>
            <button class="add-to-cart-button" @click="store.adds(dataDetailDevice.deviceId, dataDetailDevice,quantity)">ADD TO
              CART</button>
          </div>
        </div>
      </div>
...
const { data } = await useFetchApi(`/device/getbyid/${itemId}&${warehouseId.selectWarehouse}`, {
  method: 'GET',
});
if (data.value) {
